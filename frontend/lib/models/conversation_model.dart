...
class ConversationModel {
  ...
  factory ConversationModel.fromJson(Map<String, dynamic> json) {
    final unreadCount =
        JsonHelpers.parseInt(json['unread_count']) ?? 0;

    return ConversationModel(
      id: JsonHelpers.parseInt(json['id']),
      userId: JsonHelpers.parseInt(json['user_id']),
      therapistUserId: JsonHelpers.parseInt(
      ...
      therapistName: JsonHelpers.parseString(
        json['therapist_name'],
      ),
      interlocutorName: JsonHelpers.parseString(
        json['interlocutor_name'],
      ),

      lastMessageText: JsonHelpers.parseString(
            json['last_message_text'],
          ) ??
          JsonHelpers.parseString(
            json['last_message'],
          ),
      lastMessageSenderId: JsonHelpers.parseInt(
        json['last_message_sender_id'],
      ),
      unreadCount: unreadCount,
      hasUnread: _parseBool(
            json['has_unread'],
          ) ??
          unreadCount > 0,
    );
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'therapist_last_read_at':
          therapistLastReadAt?.toIso8601String(),
      'user_name': userName,
...
class ConversationMessageModel {
  ...
  factory ConversationMessageModel.fromJson(
    ...
    return ConversationMessageModel(
      ...
      content: JsonHelpers.parseString(
        json['content'],
      ),
